/*
    MPFR C++: Multi-precision floating point number class
   for C++.
    ...
    Helmut Jarausch, Fokko Beekhof, Ulrich Mutze, Heinz van
   Saanen,
    Pere Constans, Peter van Hoof, Gael Guennebaud, Tsai
   Chia Cheng,
    Alexei Zubanov, Jauhien Piatlicki, Victor Berger, John
   Westwood,
    Petr Aleksandrov, Orion Poplawski, Charles Karney, Arash
   Partow,
    Rodney James, Jorge Leitao.

    Licensing:
    (A) MPFR C++ is under GNU General Public License
   ("GPL").

    (B) Non-free licenses may also be purchased from the
   author, for users who
        do not want their programs protected by the GPL.

        The non-free licenses are for users that wish to use
   MPFR C++ in
        their products but are unwilling to release their
   software
        under the GPL (which would require them to release
   source code
        and allow free redistribution).

        Such users can purchase an unlimited-use license
   from the author.
        Contact us for more details.

    GNU General Public License ("GPL") copyright permissions
   statement:
    **************************************************************************
...
#define MPREAL_HAVE_MSVC_DEBUGVIEW  // Enable Debugger
                                    ...
                                    // "Debug" builds in
                                    // MSVC.
#define MPREAL_HAVE_DYNAMIC_STD_NUMERIC_LIMITS  // Enable
                                                // extended
// std::numeric_limits<mpfr::mpreal>
// specialization.
// Meaning that "digits", "round_style" and similar members
// are defined as functions, not constants.
// See std::numeric_limits<mpfr::mpreal> at the end of the
...
// Detect compiler using signatures from
...
#else
#define IsInf(x) \
  std::isinf(x)  // GNU C/C++ (and/or other compilers), just
// hope for C99 conformance
#endif
...
// Use fields in mpfr_t structure to check if it was
// initialized / set dummy initialization
...
#if defined(MPREAL_HAVE_MSVC_DEBUGVIEW) && \
...
#define MPREAL_DOUBLE_BITS_OVERFLOW \
  -1  // Triggers overflow exception during conversion to
      // double if mpreal
      // cannot fit in MPREAL_DOUBLE_BITS_OVERFLOW bits
      // = -1 disables overflow checks (default)

// Fast replacement for mpfr_set_zero(x, +1):
// (a) uses low-level data members, might not be compatible
// with new versions of MPFR
...
class mpreal {
 ...
 public:
  ...
  // shared = true allows to avoid deep copy, so that mpreal
  // and 'u' share the same data & pointers.
  ...
  mpreal &operator-=(const int u);
  const mpreal operator-() const;
  friend const mpreal operator-(const unsigned long int b,
                                const mpreal &a);
  friend const mpreal operator-(const unsigned int b,
                                const mpreal &a);
  friend const mpreal operator-(const long int b,
                                const mpreal &a);
  ...
  // Get raw pointers so that mpreal can be directly used in
  // raw mpfr_* functions
  ::mpfr_ptr mpfr_ptr();
  ::mpfr_srcptr mpfr_ptr() const;
  ::mpfr_srcptr mpfr_srcptr() const;

  // Convert mpreal to string with n significant digits in
  // base b
  ...
  friend const mpreal log2(const mpreal &v,
                           mp_rnd_t rnd_mode);
  friend const mpreal logb(const mpreal &v,
                           mp_rnd_t rnd_mode);
  ...
  friend const mpreal gamma(const mpreal &v,
                            mp_rnd_t rnd_mode);
  friend const mpreal tgamma(const mpreal &v,
                             mp_rnd_t rnd_mode);
  friend const mpreal lngamma(const mpreal &v,
                              mp_rnd_t rnd_mode);
  friend const mpreal lgamma(const mpreal &v, int *signp,
                             mp_rnd_t rnd_mode);
...
// MPFR 2.4.0 Specifics
#if(MPFR_VERSION >= MPFR_VERSION_NUM(2, 4, 0))
  friend int sinh_cosh(mpreal &s, mpreal &c,
  ...
  friend const mpreal urandom(
      ...
      mp_rnd_t rnd_mode);  // use gmp_randinit_default() to
                           // init state, gmp_randclear() to
                           // clear
#endif
  ...
  // Uniformly distributed random number generation in [0,1]
  // using
  // Mersenne-Twister algorithm by default.
  // Use parameter to setup seed, e.g.:
  ...
  // Constants
  // don't forget to call mpfr_free_cache() for every thread
  // where you are using const-functions
  friend const mpreal const_log2(mp_prec_t prec,
                                 mp_rnd_t rnd_mode);
  friend const mpreal const_pi(mp_prec_t prec,
                               mp_rnd_t rnd_mode);
  friend const mpreal const_euler(mp_prec_t prec,
                                  mp_rnd_t rnd_mode);
  friend const mpreal const_catalan(mp_prec_t prec,
                                    mp_rnd_t rnd_mode);

  // returns +inf iff sign>=0 otherwise -inf
  friend const mpreal const_infinity(int sign,
                                     mp_prec_t prec);
  ...
  friend const mpreal remainder(const mpreal &x,
                                const mpreal &y,
                                mp_rnd_t rnd_mode);
  friend const mpreal remquo(long *q, const mpreal &x,
                             const mpreal &y,
                             mp_rnd_t rnd_mode);

  // Miscellaneous Functions
  friend const mpreal nexttoward(const mpreal &x,
                                 const mpreal &y);
  friend const mpreal nextabove(const mpreal &x);
  friend const mpreal nextbelow(const mpreal &x);

  // use gmp_randinit_default() to init state,
...
// MPFR < 2.4.2 Specifics
  ...
  inline void set_prec(
      mp_prec_t prec,
      mp_rnd_t rnd_mode = get_default_rnd());  // Change
                                               // precision
                                               // with
                                               // rounding
                                               // mode

  // Aliases for get_prec(), set_prec() - needed for
  // compatibility with std::complex<mpreal> interface
  inline mpreal &setPrecision(
      int Precision,
  ...
  mpreal &setSign(
      int Sign, mp_rnd_t RoundingMode = get_default_rnd());
  ...
  // Efficient swapping of two mpreal values - needed for
  // std algorithms
 ...
 private:
  ...
  // Show value only
  // mpfr::mpreal=<DebugView>, <mp[0]._mpfr_prec,u>bits    ;
  // Show value & precision
  //
  // at the beginning of
  // [Visual Studio Installation
  // Folder]\Common7\Packages\Debugger\autoexp.dat
  MPREAL_MSVC_DEBUGVIEW_DATA

  // "Smart" resources deallocation. Checks if instance
  // initialized before deletion.
  void clear(::mpfr_ptr);
};
...
inline mpreal::mpreal(const mpreal &u) {
  ...
  mpfr_set(mpfr_ptr(), u.mpfr_srcptr(),
...
inline mpreal::mpreal(mpreal &&other) {
  mpfr_set_uninitialized(mpfr_ptr());  // make sure "other"
                                       // holds no pointer
                                       // to actual data
...
inline mpreal::mpreal(const mpz_t u, mp_prec_t prec,
                      mp_rnd_t mode) {
  mpfr_init2(mpfr_ptr(), prec);
...
inline mpreal::mpreal(const mpq_t u, mp_prec_t prec,
                      mp_rnd_t mode) {
  mpfr_init2(mpfr_ptr(), prec);
...
inline mpreal::mpreal(const long double u, mp_prec_t prec,
                      mp_rnd_t mode) {
  mpfr_init2(mpfr_ptr(), prec);
...
inline mpreal::mpreal(const long long int u, mp_prec_t prec,
                      mp_rnd_t mode) {
  mpfr_init2(mpfr_ptr(), prec);
...
inline mpreal::mpreal(const unsigned long int u,
                      mp_prec_t prec, mp_rnd_t mode) {
  mpfr_init2(mpfr_ptr(), prec);
...
inline mpreal::mpreal(const unsigned int u, mp_prec_t prec,
                      mp_rnd_t mode) {
  mpfr_init2(mpfr_ptr(), prec);
  mpfr_set_ui(mpfr_ptr(), u, mode);
...
inline mpreal::mpreal(const long int u, mp_prec_t prec,
                      mp_rnd_t mode) {
  mpfr_init2(mpfr_ptr(), prec);
...
inline mpreal::mpreal(const int u, mp_prec_t prec,
                      mp_rnd_t mode) {
  mpfr_init2(mpfr_ptr(), prec);
  mpfr_set_si(mpfr_ptr(), u, mode);
...
inline mpreal::mpreal(const char *s, mp_prec_t prec,
...
inline mpreal::mpreal(const std::string &s, mp_prec_t prec,
                      int base, mp_rnd_t mode) {
  mpfr_init2(mpfr_ptr(), prec);
...
// Use SFINAE to restrict arithmetic operations
// instantiation only for numeric types
// This is needed for smooth integration with libraries
// based on expression templates, like Eigen.
...
// pow
const mpreal pow(
    const mpreal &a, const unsigned int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const mpreal &a, const int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const mpreal &a, const long double b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const mpreal &a, const double b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());

const mpreal pow(
    const unsigned int a, const mpreal &b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const long int a, const mpreal &b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const int a, const mpreal &b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const long double a, const mpreal &b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const double a, const mpreal &b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());

const mpreal pow(
    const unsigned long int a, const unsigned int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const unsigned long int a, const long int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const unsigned long int a, const int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const unsigned long int a, const long double b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const unsigned long int a, const double b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());

const mpreal pow(
    const unsigned int a, const unsigned long int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const unsigned int a, const unsigned int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const unsigned int a, const long int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const unsigned int a, const int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const unsigned int a, const long double b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const unsigned int a, const double b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());

const mpreal pow(
    const long int a, const unsigned long int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const long int a, const unsigned int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const long int a, const long int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const long int a, const int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const long int a, const long double b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const long int a, const double b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());

const mpreal pow(
    const int a, const unsigned long int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const int a, const unsigned int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const int a, const long int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const int a, const int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const int a, const long double b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
...
const mpreal pow(
    ...
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const long double a, const unsigned long int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const long double a, const unsigned int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const long double a, const long int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const long double a, const int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
...
const mpreal pow(
    ...
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const double a, const unsigned long int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const double a, const unsigned int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const double a, const long int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(
    const double a, const int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());

inline const mpreal mul_2ui(
    const mpreal &v, unsigned long int k,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
inline const mpreal mul_2si(
    const mpreal &v, long int k,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
inline const mpreal div_2ui(
    const mpreal &v, unsigned long int k,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
inline const mpreal div_2si(
    const mpreal &v, long int k,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd());
...
inline mpreal machine_epsilon(
    mp_prec_t prec = mpreal::get_default_prec());

// Returns smallest eps such that x + eps != x (relative
// machine epsilon)
...
inline mpreal minval(
    mp_prec_t prec = mpreal::get_default_prec());
inline mpreal maxval(
    mp_prec_t prec = mpreal::get_default_prec());
...
// 'Dirty' equality check 2: |a-b| < min{|a|,|b|} * eps(
// min{|a|,|b|} )
inline bool isEqualFuzzy(const mpreal &a, const mpreal &b);

// 'Bitwise' equality check
//  maxUlps - a and b can be apart by maxUlps binary
//  numbers.
inline bool isEqualUlps(const mpreal &a, const mpreal &b,
                        int maxUlps);

//////////////////////////////////////////////////////////////////////////
// Convert precision in 'bits' to decimal digits and vice
// versa.
...
inline mpreal &mpreal::operator=(const mpreal &v) {
  if(this != &v) {
    ...
    mpfr_set(mpfr_ptr(), v.mpfr_srcptr(),
             mpreal::get_default_rnd());
...
inline mpreal &mpreal::operator=(const char *s) {
  // Use other converters for more precise control on base &
  // precision & rounding:
  //
  //        mpreal(const char* s,        mp_prec_t prec, int
  //        base, mp_rnd_t mode)
  //        mpreal(const std::string& s,mp_prec_t prec, int
  //        base, mp_rnd_t mode)
  //
  // Here we assume base = 10 and we use precision of target
  // variable.
  ...
  if(0 ==
     mpfr_set_str(t, s, 10, mpreal::get_default_rnd())) {
...
inline mpreal &mpreal::operator=(const std::string &s) {
  ...
  if(0 == mpfr_set_str(t, s.c_str(), 10,
                       mpreal::get_default_rnd())) {
    mpfr_set(mpfr_ptr(), t, mpreal::get_default_rnd());
    MPREAL_MSVC_DEBUGVIEW_CODE;
  }
...
inline mpreal &mpreal::operator+=(const mpz_t u) {
  mpfr_add_z(mpfr_ptr(), mpfr_srcptr(), u,
             mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
inline mpreal &mpreal::operator+=(const mpq_t u) {
  mpfr_add_q(mpfr_ptr(), mpfr_srcptr(), u,
             mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
#if(MPFR_VERSION >= MPFR_VERSION_NUM(2, 4, 0))
  mpfr_add_d(mpfr_ptr(), mpfr_srcptr(), u,
             mpreal::get_default_rnd());
#else
  *this += mpreal(u);
#endif
...
inline mpreal &mpreal::operator+=(const long int u) {
  mpfr_add_si(mpfr_ptr(), mpfr_srcptr(), u,
              mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
inline const mpreal operator+(const mpreal &a,
                              const mpreal &b) {
...
inline mpreal &mpreal::operator-=(const mpz_t v) {
  mpfr_sub_z(mpfr_ptr(), mpfr_srcptr(), v,
             mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
inline mpreal &mpreal::operator-=(const mpq_t v) {
  mpfr_sub_q(mpfr_ptr(), mpfr_srcptr(), v,
             mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
inline mpreal &mpreal::operator-=(const double v) {
#if(MPFR_VERSION >= MPFR_VERSION_NUM(2, 4, 0))
  mpfr_sub_d(mpfr_ptr(), mpfr_srcptr(), v,
             mpreal::get_default_rnd());
#else
  *this -= mpreal(v);
#endif
...
inline mpreal &mpreal::operator-=(const long int v) {
  mpfr_sub_si(mpfr_ptr(), mpfr_srcptr(), v,
              mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
inline const mpreal mpreal::operator-() const {
  mpreal u(*this);
  mpfr_neg(u.mpfr_ptr(), u.mpfr_srcptr(),
           mpreal::get_default_rnd());
  return u;
}
...
inline const mpreal operator-(const double b,
                              const mpreal &a) {
#if(MPFR_VERSION >= MPFR_VERSION_NUM(2, 4, 0))
  mpreal x(0, mpfr_get_prec(a.mpfr_ptr()));
  mpfr_d_sub(x.mpfr_ptr(), b, a.mpfr_srcptr(),
             mpreal::get_default_rnd());
  return x;
#else
...
inline const mpreal operator-(const unsigned long int b,
                              const mpreal &a) {
  mpreal x(0, mpfr_get_prec(a.mpfr_ptr()));
  mpfr_ui_sub(x.mpfr_ptr(), b, a.mpfr_srcptr(),
              mpreal::get_default_rnd());
  return x;
}

inline const mpreal operator-(const unsigned int b,
                              const mpreal &a) {
  mpreal x(0, mpfr_get_prec(a.mpfr_ptr()));
  mpfr_ui_sub(x.mpfr_ptr(), b, a.mpfr_srcptr(),
              mpreal::get_default_rnd());
...
inline const mpreal operator-(const long int b,
                              const mpreal &a) {
  mpreal x(0, mpfr_get_prec(a.mpfr_ptr()));
  mpfr_si_sub(x.mpfr_ptr(), b, a.mpfr_srcptr(),
              mpreal::get_default_rnd());
  return x;
}

inline const mpreal operator-(const int b,
                              const mpreal &a) {
  mpreal x(0, mpfr_get_prec(a.mpfr_ptr()));
  mpfr_si_sub(x.mpfr_ptr(), b, a.mpfr_srcptr(),
              mpreal::get_default_rnd());
...
inline mpreal &mpreal::operator*=(const mpz_t v) {
  mpfr_mul_z(mpfr_ptr(), mpfr_srcptr(), v,
             mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
inline mpreal &mpreal::operator*=(const mpq_t v) {
  mpfr_mul_q(mpfr_ptr(), mpfr_srcptr(), v,
             mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
inline mpreal &mpreal::operator*=(const double v) {
#if(MPFR_VERSION >= MPFR_VERSION_NUM(2, 4, 0))
  mpfr_mul_d(mpfr_ptr(), mpfr_srcptr(), v,
             mpreal::get_default_rnd());
#else
  *this *= mpreal(v);
#endif
...
inline mpreal &mpreal::operator*=(
    const unsigned long int v) {
  mpfr_mul_ui(mpfr_ptr(), mpfr_srcptr(), v,
              mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
inline mpreal &mpreal::operator*=(const long int v) {
  mpfr_mul_si(mpfr_ptr(), mpfr_srcptr(), v,
              mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
inline mpreal &mpreal::operator/=(const mpz_t v) {
  mpfr_div_z(mpfr_ptr(), mpfr_srcptr(), v,
             mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
inline mpreal &mpreal::operator/=(const mpq_t v) {
  mpfr_div_q(mpfr_ptr(), mpfr_srcptr(), v,
             mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
inline mpreal &mpreal::operator/=(const double v) {
#if(MPFR_VERSION >= MPFR_VERSION_NUM(2, 4, 0))
  mpfr_div_d(mpfr_ptr(), mpfr_srcptr(), v,
             mpreal::get_default_rnd());
#else
  *this /= mpreal(v);
#endif
...
inline mpreal &mpreal::operator/=(
    const unsigned long int v) {
  mpfr_div_ui(mpfr_ptr(), mpfr_srcptr(), v,
              mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
inline mpreal &mpreal::operator/=(const long int v) {
  mpfr_div_si(mpfr_ptr(), mpfr_srcptr(), v,
              mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
inline const mpreal operator/(const unsigned long int b,
  ...
  mpfr_ui_div(x.mpfr_ptr(), b, a.mpfr_srcptr(),
              mpreal::get_default_rnd());
  return x;
}
...
inline const mpreal operator/(const long int b,
                              const mpreal &a) {
  mpreal x(0, mpfr_get_prec(a.mpfr_srcptr()));
  mpfr_si_div(x.mpfr_ptr(), b, a.mpfr_srcptr(),
              mpreal::get_default_rnd());
  return x;
}
...
#if(MPFR_VERSION >= MPFR_VERSION_NUM(2, 4, 0))
  mpreal x(0, mpfr_get_prec(a.mpfr_srcptr()));
  mpfr_d_div(x.mpfr_ptr(), b, a.mpfr_srcptr(),
             mpreal::get_default_rnd());
  return x;
#else
  mpreal x(0, mpfr_get_prec(a.mpfr_ptr()));
...
inline mpreal &mpreal::operator<<=(
    const unsigned long int u) {
  mpfr_mul_2ui(mpfr_ptr(), mpfr_srcptr(), u,
               mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
inline mpreal &mpreal::operator<<=(const unsigned int u) {
  mpfr_mul_2ui(mpfr_ptr(), mpfr_srcptr(),
               static_cast<unsigned long int>(u),
               mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
inline mpreal &mpreal::operator<<=(const long int u) {
  mpfr_mul_2si(mpfr_ptr(), mpfr_srcptr(), u,
               mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
inline mpreal &mpreal::operator>>=(
    const unsigned long int u) {
  mpfr_div_2ui(mpfr_ptr(), mpfr_srcptr(), u,
               mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
inline mpreal &mpreal::operator>>=(const long int u) {
  mpfr_div_2si(mpfr_ptr(), mpfr_srcptr(), u,
               mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
inline const mpreal operator<<(const mpreal &v,
...
// WARNING:
//
// Please note that following checks for double-NaN are
// guaranteed to work only in IEEE math mode:
//
// isnan(b) =  (b != b)
// isnan(b) = !(b == b)  (we use in code below)
//
// Be cautions if you use compiler options which break
// strict IEEE compliance (e.g. -ffast-math in GCC).
// Use std::isnan instead (C++11).

inline bool operator>(const mpreal &a, const mpreal &b) {
  return (mpfr_greater_p(a.mpfr_srcptr(),
                         b.mpfr_srcptr()) != 0);
}
inline bool operator>(const mpreal &a,
                      const unsigned long int b) {
...
inline bool operator<=(const mpreal &a, const mpreal &b) {
  return (mpfr_lessequal_p(a.mpfr_srcptr(),
                           b.mpfr_srcptr()) != 0);
}
inline bool operator<=(const mpreal &a,
                       const unsigned long int b) {
...
inline unsigned long long mpreal::toULLong(
    mp_rnd_t mode) const {
...
inline std::string mpreal::toString(
    const std::string &format) const {
  char *s = NULL;
  std::string out;

  if(!format.empty()) {
    if(!(mpfr_asprintf(&s, format.c_str(), mpfr_srcptr()) <
         0)) {
      out = std::string(s);
...
inline std::string mpreal::toString(int n, int b,
  ...
  int digits = (n >= 0) ? n : 1 + bits2digits(mpfr_get_prec(
                                      mpfr_srcptr()));
  ...
  ns = mpfr_get_str(NULL, &exp, b, (std::max)(0, n), mp,
                    mode);
  ...
  if(s != NULL && ns != NULL) {
    ...
    if(exp > 0 && static_cast<size_t>(exp) < slen) {
      ...
      if(--exp) {
        if(exp > 0)
          out += "e+" +
                 mpfr::toString<mp_exp_t>(exp, std::dec);
        else
          out +=
              "e" + mpfr::toString<mp_exp_t>(exp, std::dec);
      }
    }
...
//////////////////////////////////////////////////////////////////////////
// I/O
inline std::ostream &mpreal::output(
...
inline mpreal &mpreal::setSign(int sign,
                               mp_rnd_t RoundingMode) {
...
inline mpreal &mpreal::setPrecision(int Precision,
                                    mp_rnd_t RoundingMode) {
...
inline void mpreal::set_prec(mp_prec_t prec,
                             mp_rnd_t rnd_mode) {
...
inline mpreal maxval(mp_prec_t prec) {
  ...
  return (mpreal(1, prec) - machine_epsilon(prec))
         << mpreal::get_emax();
}
...
inline bool isEqualUlps(const mpreal &a, const mpreal &b,
                        int maxUlps) {
  return abs(a - b) <=
         machine_epsilon((max)(abs(a), abs(b))) * maxUlps;
}
...
inline bool isEqualFuzzy(const mpreal &a, const mpreal &b) {
  return isEqualFuzzy(a, b, machine_epsilon((max)(
                                1, (min)(abs(a), abs(b)))));
}

//////////////////////////////////////////////////////////////////////////
// C++11 sign functions.
inline mpreal copysign(
    const mpreal &x, const mpreal &y,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  mpreal rop(0, mpfr_get_prec(x.mpfr_ptr()));
  mpfr_setsign(rop.mpfr_ptr(), x.mpfr_srcptr(),
               mpfr_signbit(y.mpfr_srcptr()), rnd_mode);
  return rop;
}
...
inline const mpreal modf(const mpreal &v, mpreal &n) {
  ...
  // rounding is not important since we are using the same
  // number
  mpfr_frac(f.mpfr_ptr(), f.mpfr_srcptr(),
            mpreal::get_default_rnd());
...
inline const mpreal sqrt(const unsigned int v,
                         mp_rnd_t rnd_mode) {
  return sqrt(static_cast<unsigned long int>(v), rnd_mode);
}

inline const mpreal sqrt(const long int v,
                         mp_rnd_t rnd_mode) {
  if(v >= 0)
...
inline const mpreal root(
    const mpreal &x, unsigned long int k,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  mpreal y(0, mpfr_get_prec(x.mpfr_srcptr()));
...
inline const mpreal dim(
    const mpreal &a, const mpreal &b,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  mpreal y(0, mpfr_get_prec(a.mpfr_srcptr()));
  mpfr_dim(y.mpfr_ptr(), a.mpfr_srcptr(), b.mpfr_srcptr(),
           r);
  return y;
}
...
inline int sin_cos(
    mpreal &s, mpreal &c, const mpreal &v,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  return mpfr_sin_cos(s.mpfr_ptr(), c.mpfr_ptr(),
                      v.mpfr_srcptr(), rnd_mode);
}

inline const mpreal sqrt(const long double v,
                         mp_rnd_t rnd_mode) {
  return sqrt(mpreal(v), rnd_mode);
}
inline const mpreal sqrt(const double v,
                         mp_rnd_t rnd_mode) {
  return sqrt(mpreal(v), rnd_mode);
}

inline const mpreal cbrt(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
...
inline const mpreal asec(
    const mpreal &v,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  return acos(1 / v, r);
}
inline const mpreal acsc(
    const mpreal &v,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  return asin(1 / v, r);
}
inline const mpreal acoth(
    const mpreal &v,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  return atanh(1 / v, r);
}
inline const mpreal asech(
    const mpreal &v,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  return acosh(1 / v, r);
}
inline const mpreal acsch(
    const mpreal &v,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  return asinh(1 / v, r);
}

inline const mpreal cosh(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(cosh);
}
inline const mpreal sinh(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(sinh);
}
inline const mpreal tanh(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(tanh);
}
inline const mpreal sech(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(sech);
}
inline const mpreal csch(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(csch);
}
inline const mpreal coth(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(coth);
}
inline const mpreal acosh(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(acosh);
}
inline const mpreal asinh(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(asinh);
}
inline const mpreal atanh(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(atanh);
}

inline const mpreal log1p(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(log1p);
}
inline const mpreal expm1(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(expm1);
}
inline const mpreal eint(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(eint);
}
inline const mpreal gamma(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(gamma);
}
inline const mpreal tgamma(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(gamma);
}
inline const mpreal lngamma(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(lngamma);
}
inline const mpreal zeta(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(zeta);
}
inline const mpreal erf(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(erf);
}
inline const mpreal erfc(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(erfc);
}
inline const mpreal besselj0(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(j0);
}
inline const mpreal besselj1(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(j1);
}
inline const mpreal bessely0(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(y0);
}
inline const mpreal bessely1(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(y1);
}

inline const mpreal atan2(
    const mpreal &y, const mpreal &x,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  mpreal a(0,
           (std::max)(y.getPrecision(), x.getPrecision()));
  mpfr_atan2(a.mpfr_ptr(), y.mpfr_srcptr(), x.mpfr_srcptr(),
             rnd_mode);
  return a;
}

inline const mpreal hypot(
    const mpreal &x, const mpreal &y,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  mpreal a(0,
           (std::max)(y.getPrecision(), x.getPrecision()));
  mpfr_hypot(a.mpfr_ptr(), x.mpfr_srcptr(), y.mpfr_srcptr(),
             rnd_mode);
  return a;
}

inline const mpreal remainder(
    const mpreal &x, const mpreal &y,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  mpreal a(0,
           (std::max)(y.getPrecision(), x.getPrecision()));
  mpfr_remainder(a.mpfr_ptr(), x.mpfr_srcptr(),
                 y.mpfr_srcptr(), rnd_mode);
  return a;
}

inline const mpreal remquo(
    long *q, const mpreal &x, const mpreal &y,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  mpreal a(0,
           (std::max)(y.getPrecision(), x.getPrecision()));
  mpfr_remquo(a.mpfr_ptr(), q, x.mpfr_srcptr(),
              y.mpfr_srcptr(), rnd_mode);
  return a;
}

inline const mpreal fac_ui(
    unsigned long int v,
    mp_prec_t prec = mpreal::get_default_prec(),
    mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  mpreal x(0, prec);
...
inline const mpreal lgamma(
    const mpreal &v, int *signp = 0,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  mpreal x(v);
  int tsignp;

  if(signp)
    mpfr_lgamma(x.mpfr_ptr(), signp, v.mpfr_srcptr(),
                rnd_mode);
  else
    mpfr_lgamma(x.mpfr_ptr(), &tsignp, v.mpfr_srcptr(),
                rnd_mode);

  return x;
}

inline const mpreal besseljn(
    long n, const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
...
inline int sinh_cosh(
    mpreal &s, mpreal &c, const mpreal &v,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  return mpfr_sinh_cosh(s.mp, c.mp, v.mp, rnd_mode);
}

inline const mpreal li2(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(li2);
}

inline const mpreal rem(
    const mpreal &x, const mpreal &y,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  /*  R = rem(X,Y) if Y != 0, returns X - n * Y where n =
   * trunc(X/Y). */
  return fmod(x, y, rnd_mode);
}

inline const mpreal mod(
    const mpreal &x, const mpreal &y,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  (void)rnd_mode;
  ...
  m = mod(x,y) if y != 0, returns x - n*y where n =
  floor(x/y)
  ...
  */
  ...
  m.setSign(
      sgn(y));  // make sure result has the same sign as Y
...
#if(MPFR_VERSION >= MPFR_VERSION_NUM(3, 0, 0))
inline const mpreal digamma(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(digamma);
}
inline const mpreal ai(
    const mpreal &x,
    mp_rnd_t r = mpreal::get_default_rnd()) {
...
//////////////////////////////////////////////////////////////////////////
// Constants
inline const mpreal const_log2(
...
inline const mpreal const_euler(
    mp_prec_t p = mpreal::get_default_prec(),
    mp_rnd_t r = mpreal::get_default_rnd()) {
  mpreal x(0, p);
...
inline const mpreal const_catalan(
    mp_prec_t p = mpreal::get_default_prec(),
    mp_rnd_t r = mpreal::get_default_rnd()) {
  mpreal x(0, p);
...
inline const mpreal const_infinity(
    int sign = 1,
    mp_prec_t p = mpreal::get_default_prec()) {
  mpreal x(0, p);
...
inline const mpreal nexttoward(const mpreal &x,
                               const mpreal &y) {
  mpreal a(x);
...
inline bool mpreal::fits_in_bits(double x, int n) {
  ...
  double t;
  return IsInf(x) ||
         (std::modf(std::ldexp(std::frexp(x, &i), n), &t) ==
          0.0);
}

inline const mpreal pow(
    const mpreal &a, const mpreal &b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  mpreal x(a);
...
inline const mpreal pow(
    const mpreal &a, const unsigned long int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  mpreal x(a);
...
inline const mpreal pow(
    const mpreal &a, const long int b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  mpreal x(a);
...
inline const mpreal pow(const mpreal &a, const int b,
                        mp_rnd_t rnd_mode) {
...
inline const mpreal pow(const mpreal &a, const double b,
                        mp_rnd_t rnd_mode) {
  return pow(a, mpreal(b), rnd_mode);
}

inline const mpreal pow(
    const unsigned long int a, const mpreal &b,
    mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  mpreal x(a);
...
inline const mpreal pow(const long int a, const mpreal &b,
                        mp_rnd_t rnd_mode) {
  if(a >= 0)
    return pow(static_cast<unsigned long int>(a), b,
               rnd_mode);
...
inline const mpreal pow(const int a, const mpreal &b,
                        mp_rnd_t rnd_mode) {
  if(a >= 0)
    return pow(static_cast<unsigned long int>(a), b,
               rnd_mode);
...
inline const mpreal pow(const double a, const mpreal &b,
                        mp_rnd_t rnd_mode) {
  return pow(mpreal(a), b, rnd_mode);
}
...
inline const mpreal pow(const unsigned int a, const int b,
                        mp_rnd_t rnd_mode) {
  if(b > 0)
    return pow(static_cast<unsigned long int>(a),
               static_cast<unsigned long int>(b),
...
inline const mpreal pow(const long int a, const long int b,
                        mp_rnd_t rnd_mode) {
  if(a > 0) {
    if(b > 0)
      return pow(static_cast<unsigned long int>(a),
      ...
      return pow(static_cast<unsigned long int>(a),
                 mpreal(b), rnd_mode);  // mpfr_ui_pow
  } else {
    return pow(mpreal(a), b, rnd_mode);  // mpfr_pow_si
  }
}

inline const mpreal pow(const long int a, const int b,
                        mp_rnd_t rnd_mode) {
  if(a > 0) {
    if(b > 0)
      return pow(static_cast<unsigned long int>(a),
...
inline const mpreal pow(const long int a, const double b,
                        mp_rnd_t rnd_mode) {
  if(a >= 0)
    return pow(static_cast<unsigned long int>(a), mpreal(b),
               rnd_mode);  // mpfr_ui_pow
...
inline const mpreal pow(const int a, const unsigned int b,
                        mp_rnd_t rnd_mode) {
  if(a > 0)
    return pow(static_cast<unsigned long int>(a),
               static_cast<unsigned long int>(b),
...
inline const mpreal pow(const int a, const long int b,
                        mp_rnd_t rnd_mode) {
  if(a > 0) {
    if(b > 0)
      return pow(static_cast<unsigned long int>(a),
...
inline const mpreal pow(const int a, const int b,
                        mp_rnd_t rnd_mode) {
  if(a > 0) {
    if(b > 0)
      return pow(static_cast<unsigned long int>(a),
...
inline const mpreal pow(const int a, const long double b,
                        mp_rnd_t rnd_mode) {
  if(a >= 0)
    return pow(static_cast<unsigned long int>(a), mpreal(b),
               rnd_mode);  // mpfr_ui_pow
...
inline const mpreal pow(const int a, const double b,
                        mp_rnd_t rnd_mode) {
  if(a >= 0)
    return pow(static_cast<unsigned long int>(a), mpreal(b),
               rnd_mode);  // mpfr_ui_pow
...
inline const mpreal pow(const long double a, const int b,
                        mp_rnd_t rnd_mode) {
  return pow(mpreal(a), static_cast<long int>(b),
             rnd_mode);  // mpfr_pow_si
}

inline const mpreal pow(const double a, const double b,
                        mp_rnd_t rnd_mode) {
  return pow(mpreal(a), mpreal(b), rnd_mode);
}
...
inline const mpreal pow(const double a, const long int b,
                        mp_rnd_t rnd_mode) {
  return pow(mpreal(a), b, rnd_mode);  // mpfr_pow_si
}

inline const mpreal pow(const double a, const int b,
                        mp_rnd_t rnd_mode) {
  return pow(mpreal(a), static_cast<long int>(b),
             rnd_mode);  // mpfr_pow_si
}
...
// Thus standard algorithms will use efficient version of
// swap (due to Koenig lookup)
// Non-throwing swap C++ idiom:
// http://en.wikibooks.org/wiki/More_C%2B%2B_Idioms/Non-throwing_swap
namespace std {
// we are allowed to extend namespace std with
...
class numeric_limits<mpfr::mpreal> {
 public:
  ...
  // Returns smallest eps such that 1 + eps != 1 (classic
  // machine epsilon)
  ...
  // Following members should be constant according to
  // standard, but they can be variable in MPFR
  // So we define them as functions here.
  //
  // This is preferable way for
  // std::numeric_limits<mpfr::mpreal> specialization.
  // But it is incompatible with standard
  // std::numeric_limits and might not work with other
  // libraries, e.g. boost.
...
#else
  // Digits and round_style are NOT constants when it comes
  // to mpreal.
  // If possible, please use functions digits() and
  // round_style() defined above.
  //
  // These (default) values are preserved for compatibility
  // with existing libraries, e.g. boost.
  // Change them accordingly to your application.
  //
  // For example, if you use 256 bits of precision uniformly
  // in your program, then:
  // digits       = 256
  // digits10     = 77
  // max_digits10 = 78
  //
  // Approximate formula for decimal digits is: digits10 =
  // floor(log10(2) * digits). See bits2digits() for more
  // details.
