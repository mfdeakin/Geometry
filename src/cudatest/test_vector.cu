...
TEST(Vector, CudaCopy) {
  ...
  std::uniform_real_distribution<fptype> pdf(minValue,
                                             maxValue);
...
__global__ void CudaAdd(V::VectorData *lhs,
                        V::VectorData *rhs,
                        V::VectorData *dest,
                        int numVectors) {
  ...
  for(int i = 0; i < vpt && threadIdx + i < numVectors;
      i++) {
...
TEST(Vector, CudaAdd) {
  ...
  for(int t = 0; t < numTests; t++) {
    ...
    CudaAdd<<<numBlocks, numThreads>>>(lhsMem, rhsMem,
                                       destMem, numVecs);
    for(int i = 0; i < numVecs; i++) {
      V sum = lhs[i] + rhs[i];
      V cmp;
